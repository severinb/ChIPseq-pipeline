...
def execute(cf):
    ...
    tfbs_string = cf.get_parameter("TFBSs_string", "string")
    peakposts_string = cf.get_parameter("peakposts_string", "string")
    ...
    for pair in tfbs_string.split(';'):
        t = pair.split()
        names.append(os.path.split(t[0])[1])
        tfbsfiles.append(t[1])

    for pair in peakposts_string.split(';'):
        t = pair.split()
        peakfiles.append(t[1])
