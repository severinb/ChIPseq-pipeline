...
def EM2(x, rho, mu, sigma, fraglen, maxDiff=0.1):
    ...
    if fraglen < 0:
        ...
        # minsig = 0.35*fraglen + 5#- 10
        # maxsig = 0.35*fraglen + 105
        #fit 0.5*fraglen+14
        minsig = 5  #max(1, 0.5*fraglen - 70) #max(1, 0.5*fraglen - 16) #-30
        maxsig = 300 #0.5*fraglen + 100 #0.5*fraglen + 44 #94 #+80
...
def main(covfile, order, outfile, outplot, fraglen, width):
    ...
    order = 1 #max(2, min(int(maxx/ (4*fraglen)), 30)) #constrain Gaussian number. Otherwise it can get very slow...
...
if __name__ == '__main__':
    """
    This program fits a mixture model of Gaussians to read coverage profiles.
    It doesn't constrain sigmas of Gaussians a lot, it nearly doesn't.
    It doesn't merge overlapping Gaussians.
    This is used to determine relationship between peak widths (sigmas) and fragment length....
    """
