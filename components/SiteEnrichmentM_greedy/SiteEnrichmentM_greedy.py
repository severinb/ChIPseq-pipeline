...
def giveMotevoParamFile(genome, inter_dir, tag, aligned, ufemodel_path, ATfreq, GCfreq, emprior, bgorder, bgprior, UFEwmprior):
    """
...
def compute_site_enrichment(train_set, test_set, background_set, tmpwm, ufemodel_path, ATfreq, GCfreq, interm, genome, motevo_path, aligned, tag_i):

    ## 1. Fit prior on training set with EM
    tag = 'fitP_' + tag_i
    params, sites, priors, loglikfile = giveMotevoParamFile(genome, interm, tag, aligned, ufemodel_path, ATfreq, GCfreq, emprior=1, bgorder=0, bgprior=0.99, UFEwmprior=200)
    ...
    tag = 'compLL_' + tag_i
    params, sites, priors, loglikfile = giveMotevoParamFile(genome, interm, tag, aligned, ufemodel_path, ATfreq, GCfreq, emprior=0, bgorder=0, bgprior=bgprior, UFEwmprior=UFEwmprior)
    ...
    seqs_n += list(zeros(input_seqs_num - len(seqs_n))) #has to be of the same length for every factor!
    seqs_n = array(seqs_n)
    seqs_n += 0.00001 #pseudo count to not get -inf in the logs for zero sites sequences.
    ...
    site_enrichment = sum(log(seqs_n)) - (len(seqs_n) * log(N)) 

    print 'Number of peaks: ', len(seqs_n)
    print 'Number of sites in background: ', N
...
def execute(cf):
    ...
    while 1:
        ...
        for i, WM in enumerate(WMs):
            ...
            tag = '%i_%i' %(j, i+1)
            site_enrichment, prior_d = compute_site_enrichment(train_set, test_set, background_set, tmpwm, ufemodel_path, ATfreq, GCfreq, interm, genome, motevo_path, aligned, tag)

            logliks.append(site_enrichment)

            if init:
                init_lls.append(site_enrichment)
        ...
        else: #for the first round always add the best matrix and continue
            final_wms_indxs.append(sorted_indxs_logliks[0][0])
            final_lls.append(sorted_indxs_logliks[0][1])
            continue
            #break
    ...
    plot(arange(len(goodwmpaths)), final_lls, 'r-')
    plot(arange(len(goodwmpaths)), final_lls, 'ko')
    ...
    xticks(arange(len(goodwmpaths)+1), names, rotation=15)
