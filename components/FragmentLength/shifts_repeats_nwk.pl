...
#		print "omitted due to repeat\n";
        ...
        #check it is NOT inside a peakds
        if ($s[5] eq "+" && !$inrepeat)
        {
            $pos = $s[1];

            if (!defined($readplus{$pos}))
            {
                $readplus{$pos} = 1;
            }

            #this was commented (3 lines). Btw, weights are not taken into account.
            else{
                if (multi == 1) {
                    ++$readplus{$pos};
                }
        ...
        elsif ($s[5] eq "-" && !$inrepeat)
        {
            $pos = $s[2];
            if (!defined($readmin{$pos}))
            {
                $readmin{$pos} = 1;
            }
    ...
    foreach $pos (keys(%readplus))
    {
        for ($relpos = 0 ; $relpos <= $maxLen ; ++$relpos)
        {
            ...
            {
                $count[$relpos] += $readplus{$pos} * $readmin{$abspos};
            }
        }
    }
