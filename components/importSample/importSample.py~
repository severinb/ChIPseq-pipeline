...
def execute(cf):
    ...
    # #input file (unique fasta file with removed adaptors and quality filtered) is in input_dir
    # infilelist = os.listdir(in_dir)
    # for inf in infilelist:
    #     unique = re.search('\.unique\.fa$', inf)
    #     if unique:
    #         infile = os.path.join(in_dir, inf)
    ...
    proc = subprocess.Popen(perlPATH + ' ' + FMIpath+"/soft/importSample.pl %s %s \'%s\' -f fasta -v" % (infile, FMIid, desc),
