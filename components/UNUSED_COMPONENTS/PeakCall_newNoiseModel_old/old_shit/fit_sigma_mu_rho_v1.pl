...
$insq = $ARGV[1]; # put -1 to eastimate sigma, otherwise give the known (positive) sigma
if(!defined($insq)){
    $insq = -1;
}
...
my $settings = LoadFile($ARGV[2]); # the yaml parameter file used to start pipeline
print STDERR "The fg window length is ", $settings->{'WINDOW'}."\n";
print STDERR "The bg window length is ", $settings->{'BACKGROUND WINDOW'}."\n";
my $ratio = $settings->{'WINDOW'} / $settings->{'BACKGROUND WINDOW'};
print STDERR "Window ratio: ", $ratio."\n";
