...
my $plotfile = $ARGV[4]; #file for histogram plot
...
while(<F>){
    ...
    $totwin++;
    if( $makecut_fg==0 || $makecut_bg==0 ) { next; } #S: Windows that don't make the cut-off have less than 2 counts: This is to few for the error model approximation to be correct (Poissonian is approximated by Gaussian)
    $passed++;
...
my $prefac = 1/$nbrfg + 1/$nbrbg;  #bug: was 1/$nbrfg**2 + 1/$nbrbg**2
...
while (abs(2.0 * ($sqmax - $sqmin) / ($sqmax + $sqmin)) > 0.01)
{
	...
	for (my $i = 0 ; $i < $num ; ++$i){
	    ...
	    my $v = 1.0 / ($sq + $sam[$i]); # usedo to be: 1/(2sigma^2 + 1/n_f + 1/n_b); sq=2sigma^2; sam= 1/n_f + 1/n_b #S: v is the whole denominator (2sigma**2+1/nf +1/nb)
	    my $dd = $sd[$i];
