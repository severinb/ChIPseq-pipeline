...
def normalizeHeights(infile, covdir, outfile, peakstatsfile, peakstatsoutfile, length):
    ...
    for line in open(infile):
        ...
        m = int((start + end)/2.0)
        ml = str(max(0, m - int(length/2.0)))
        mr = str(m + int(length/2.0))
        IDnew = '_'.join([chrom, ml, mr])


        height = float(t[-2])
        h1 = height

        regcov = os.path.join(covdir, IDnew)
        ...
        try:
            ...
            a = [1,1]
            notfound += 1
            print '%s not found (from peak: %s)' %(IDnew, ID)
...
def execute(cf):
    ...
    length = cf.get_parameter("bg_length", "int")

    T1 = datetime.datetime.now()

    normedfile = os.path.join(os.path.split(allpeaks)[0], 'normedfile')
    totnum, notfound = normalizeHeights(in_file, covdir, normedfile, peakstats, peakstatsout, length)
    ...
    figure()
    hist(log10(heights), 300)
    title('histogram of peak heights (log10-space)')
    savefig(height_hist)
    close()
