...
def execute(cf):
    """
    This CombinePeaksTFBSs_peaks function takes one TFBSstats file from refined and known as input, the same with peakstats.
    The other CombinePeaksTFBSs function takes TFBSstats and peakstats from several WMs and as parameters
    """

    peaks = cf.get_input("peaks")
    peaks_known = cf.get_input("peaks_known") 
    peaks_refined = cf.get_input("peaks_refined")
    tfbs_known = cf.get_input("TFBSs_known") 
    tfbs_refined = cf.get_input("TFBSs_refined")
    ...
    known_name = cf.get_parameter("known_name", "string")
    refined_name = known_name + '.refined'
    ...
    names = [known_name, refined_name]
    tfbsfiles = [tfbs_known, tfbs_refined]
    peakfiles = [peaks_known, peaks_refined]
    ...
    for i, tf in enumerate(peakfiles):
        WM1_d = {}
        for line in open(tf):
            if line.startswith('#'):
                continue
        ...
        WM_dicts_peaks.append(WM1_d)
        wmnames.append('%s_site_number' %(names[i]))
    ...
    for line in open(peaks):
        ...
        for j, wmdict in enumerate(WM_dicts_peaks):
            try:
                post = wmdict[peakID]
                posts.append(post)
            except KeyError:
        ...
        for j, wmdict in enumerate(WM_dicts):
            try:
                sitelist = wmdict[peakID]
                for site in sitelist:
                    sites.append('%s:%i:%.3f' %(names[j], site[0], site[1]))
            except KeyError:
                pass
