...
def ROCanalysis(non_sites_true, non_sites_bg, ref_sites_true, ref_sites_bg, ROCplot):
    """
    This function produces a ROC plot with functions from the ROC.py module.
    """

    nonref_trueSitesPost_dict = ROC2.get_peak_posteriors(non_sites_true)
    nonref_bgSitesPost_dict = ROC2.get_peak_posteriors(non_sites_bg)
    ref_trueSitesPost_dict = ROC2.get_peak_posteriors(ref_sites_true)
    ref_bgSitesPost_dict = ROC2.get_peak_posteriors(ref_sites_bg)
...
def execute(cf):
    ...
    if not WM2: #only do the refinement if there is no WM2 given
        ...
        #Create output WM with logo path in it:
    ...
    non_ref_Quality, ref_Quality = ROCanalysis(sitesNonOdd, sitesNonBg, sitesRefOdd, sitesRefBg, ROCplot)
