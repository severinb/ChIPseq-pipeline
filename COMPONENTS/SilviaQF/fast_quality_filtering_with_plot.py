...
def usage():
    ...
    print "\nUSAGE: ./thisprogram.py input_file[.fastq] output_folder_path plot_before_filepath(.pdf) plot_after_filepath(.pdf) rename_tag log_file\n"
...
class ParseFastQ(object):
    ...
    def __init__(self, file_path):
        ...
        #to check for gzipped or none gzipped fastq
        gzipped = True
        try:
...
def phred_quality_filter(inp_file, out_folder, offset, encoding):
    ...
    out_file = open(out_folder + "/" + rename + infRoot + ".fa", "w")
    parser = ParseFastQ(inp_file)
    tot_reads = 0
    good_reads = 0
    ...
    logf.write('Codification type: %s\n' %encoding)

    print 'Original number of reads: ', tot_reads
    logf.write('Original number of reads: %s\n' %tot_reads)

    print 'Reads that passed the quality filter: ', good_reads, "(% 0.2f" % \
    ((good_reads / float(tot_reads)) * 100), "%)"
    logf.write('Reads that passed the quality filter: %s (%0.2f percent)\n' %(good_reads, ((good_reads / float(tot_reads)) * 100)))

    return tot_reads, good_reads, tot_read_length_frequencies, \
           good_read_length_frequencies

def solexa_quality_filter(inp_file, out_folder, offset, encoding):
    """This function filters the reads contained in the input file assuming 
...
def plot_read_length_frequencies(plot_before, plot_after, tot_read_length_frequencies,
    ...
    r_script = open(out_folder + "/../r_script.R", "w")
...
col='blue', xlim=c(min_x,max_x), ylim=c(min_y,max_y), las=1)\n")
    ...
    os.system("R --vanilla < " + out_folder + "/../r_script.R")
