...
def ROCanalysis(FGsitefiles, BGsitefiles, ROCplot, interm):
    """
    ...
    for i in arange(len(FGsitefiles)):
        ...
        FGposteriors = ROC2.get_peak_posteriors(FGsitefiles[i])
        BGposteriors = ROC2.get_peak_posteriors(BGsitefiles[i])
        ...
        figure()
        plot(senslist, ppvlist, label='AUC: %.3f' %AUC)
        xlabel('sensitivity')
    ...
    for i in sorted(AUCdict, key= lambda k: AUCdict[k][-1], reverse=1)[:4]:
        plot(AUCdict[i][0], AUCdict[i][1], label='WM_%i' %(i+1))
...
def execute(cf):
    ...
    #     outwm = os.path.join(interm, 'WM_'+str(i))
    #     trimWM(WM, 0.25, outwm)
    #     trimmedWMs.append(outwm)
    ...
    AUCs = ROCanalysis(FGsitefiles, BGsitefiles, ROCplot, interm)


    bestAUC = 0.0
    bestindex = None
    for i in arange(len(AUCs)):
        if AUCs[i] >= bestAUC:
            bestAUC = AUCs[i]
            bestindex = i

    #replace name in WM file with bestWM
    lines = open(WMs[bestindex]).readlines()
    lines[1] = 'NA BestWM\n'
    bwm = open(bestWM, 'w')
