...
def execute(cf):
    ...
    # I think a motif can't increase log-likelihood more when combined with other motifs than just by itself. (That's way taking bad WMs out doesn't hurt)
    ...
    plot(arange(len(goodwmpaths)+1), [0] + final_lls, '-o')

    if not keepWMcontributes:
        names = ['0']+['WM_%i' %(i+1) for i in arange(len(goodwmpaths))]
        names[-1] += '\n(given)'
    else:
        names = ['0']
    ...
    names = ['WM_%i\t%s\t%.4f\t%s' %(i+1, WMs[i], init_lls[i], init_priors[i]) for i in arange(len(WMs))]

    l.write('\n'.join(names))
    l.close()


    # here write WM paths to of the WMs that were copied to the output directory, so that i$the naming is consistent afterwards.
