...
if __name__ == '__main__':
    ...
    if make_dir (params ['RESULT DIR']):
        sys.syserr.write( "Error in creating directory %s. Program quits!\n" \
              % params['RESULT DIR'])
        exit(-1)
    ...
    if len(params['INPUT FILE']) > 1:  # if there are more than one replicate, it goes on multi-threading
        ...
        for t in threads: t.join()
        result.flush()
        print 'sort -T /import/bc2/scratch/tmp/ -k 1,1 -k 2,2n -k 5,5n -u %s | awk \'{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$5}\' > %s' % \
