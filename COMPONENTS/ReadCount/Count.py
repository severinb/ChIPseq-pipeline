...
class Count:
    ...
    def read_count_in_chrom(self, Chr, initial_read):
        ...
        It, does not set the position/file_pointer in the read_file to zero"""
        ...
        if current_read.give_chrom() != Chr:
            return current_read
            # current_chr = current_read.give_chrom()
            # while(current_read.give_chrom() == current_chr):
        ...
        while (current_read.give_chrom() == Chr):             
             ...
             for window in window_array:
                 if current_read.give_start()  >  window.give_end():
                 ...
                 if current_read.give_start() >=  window.give_start() and \
                        current_read.give_end() <=  window.give_end():
