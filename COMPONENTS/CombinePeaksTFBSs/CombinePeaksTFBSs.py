...
def execute(cf):
    ...
    knowntfbsfiles = knowntfbs_string.split()
    denovotfbsfiles = denovotfbs_string.split()
    knownpeakfiles = knownpeakposts_string.split()
    denovopeakfiles = denovopeakposts_string.split()
    ...
    for i, tf in enumerate(knownpeakfiles):
        ...
        wmnames.append('knownWM_%i_summed_posteriors' %(i+1))
    ...
    for i, tf in enumerate(denovopeakfiles):
        ...
        wmnames.append('denovoWM_%i_summed_posteriors' %(i+1))
    ...
    for line in open(peaks):
        ...
        for j, wmdict in enumerate(knownWM_dicts):
            try:
                ...
                for site in sitelist:
                    sites.append('knownWM_%i:%i:%.3f' %(j+1, site[0], site[1]))
            except KeyError:
                pass
        ...
        for j, wmdict in enumerate(denovoWM_dicts):
            try:
                sitelist = wmdict[peakID]
                for site in sitelist:
                    sites.append('denovoWM_%i:%i:%.3f' %(j+1, site[0], site[1]))
            except KeyError:
                pass
