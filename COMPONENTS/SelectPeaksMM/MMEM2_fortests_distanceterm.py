...
def EM2(x, rho, mu, sigma, fraglen, weight, maxDiff=0.01):

    #weight is the weight for the distance term. 0 should be the same as running without distance term
    ...
    while True:
        ...
        ll += weight * sum( [ sum( [ (abs(mu[i]-mu[j])/(sigma[i] + sigma[j])) for j in arange(i)] ) for i in arange(mix)] ) #add the term for Gaussian closeness penalization
        ...
        # update parameter
        #mu = [sum(x*ppG[i])/sum(ppG[i]) + sum([((sigma[i] + sigma[j])**2 * sigma[i]**2)/(abs(mu[i]-mu[j]) * sum(ppG[i])) for j in arange(mix) if j != i]) for i in arange(mix)] 
        
        #mu = [   sum(x*ppG[i])/sum(ppG[i]) + sum([ ( (sigma[i]**2 * (sigma[i]+sigma[j])**2)/abs(mu[i]-mu[j]) )/ sum(ppG[i])  for j in arange(i)])    for i in arange(mix)]
        #candmu = [   sum(x*ppG[i])/sum(ppG[i]) - weight * sum([ ( (sigma[i]**2 * (sigma[i]+sigma[j]))/abs(mu[i]-mu[j]) )/ sum(ppG[i])  for j in arange(mix) if j != i])    for i in arange(mix)]
        candmu = [   sum(x*ppG[i])/sum(ppG[i]) + weight * sum([ ( (sigma[i]**2 )/abs(mu[i]-mu[j]) )/ sum(ppG[i])  for j in arange(mix) if j != i])    for i in arange(mix)]
        ...
        #candsigma = [ sum([   ( abs(mu[i]-mu[j])/((abs(mu[i]-mu[j])/sigma[i]) + sigma[j]) )/ sum(ppG[i])   for j in arange(i) ]) - (sum( ppG[i] * ((x-mu[i])**2/sigma[i]) )/sum(ppG[i]))    for i in arange(mix)]
        #candsigma = [ sum([   ( abs(mu[i]-mu[j])/((abs(mu[i]-mu[j])/sigma[i]) + sigma[j]) )/ sum(ppG[i])   for j in arange(mix) if j != i ]) + (sum( ppG[i] * ((x-mu[i])**2/sigma[i]) )/sum(ppG[i]))    for i in arange(mix)]

        candsigma = [  sqrt( sum([ppG[i] * ((x-mu[i])**2) ]) / ( weight * sum([   sigma[i] / ( sigma[i] + sigma[j] )  for j in arange(mix) if j != i]) + sum(ppG[i]) ) )  for i in arange(mix)]


        #candsigma = [  (1./sum([  abs(mu[i]-mu[j])/((sigma[i] + sigma[j])**2) * (sum(ppG[i]) +  ( mu[i]/(sigma[i]**2) * sum( ppG[i] * (x[i]/(sigma[i]**2)) ) )  )  for j in arange(mix) if j != i ]))       for i in arange(mix)]
        #print '---:', candsigma
        sigma = [min( max(minsig, candsigma[i]), maxsig) for i in arange(mix)]
...
def main(covfile, order, outfile, outplot, fraglen, width, weight):
    ...
    for ijk in arange(3): #[1,2,3,4,5]: #run MMEM 5 times. Keep track of the best fit... Best fit is defined as the one that has least RMSD over the whole region.
        ...
        rho, mu, sig, Lik = EM2(x, rho, mu, sig, fraglen, weight)
...
if __name__ == '__main__':
    ...
    main(ppath, order, outfile, outplot, fraglen, width, weight)
