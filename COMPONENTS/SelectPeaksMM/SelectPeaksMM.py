...
def submitJobs(count, infileroot, outfileroot, plotdir, fraglen, order, project_leader, width):

    JOB_PARAM = '-q fs_long -P %s -e %s/job.stderr -o %s/job.stdout -j n -N MM -cwd -V -b y' %(project_leader, os.path.split(infileroot)[0], os.path.split(infileroot)[0])
    ...
    #jt.remoteCommand = '/import/bc2/home/nimwegen/GROUP/hseq_pipeline/severin/Anduril/Pipeline/COMPONENTS/COMPONENTS.v6/SelectPeaksMM/MMEM_peaks_select2_newmerge_newextract_1Gauss_1file.py'
...
def combineFiles(count, fileroot, interm, allpeaks, statsfile, rmsd_co):
    ...
    badqualpeaksfile = os.path.join(interm, 'badqualpeaks')
    ...
    os.system('sort -k5gnr %s > %s' %(tmpfile, allpeaks))
...
def execute(cf):
    ...
    submitJobs(count, filesfileroot, statsfileroot, plotdir, fraglen, order, project_leader, width)
    ...
    figure()
    plot(log10(sorted(heights, reverse=True)), arange(1,len(heights)+1,1), '.')
    lh = log10(lastheight)
