...
def EM2(x, rho, mu, sigma, fraglen, maxDiff=0.1):
    ...
    if fraglen < 0:
        ...
        #fit 0.5*fraglen+14
        minsig = 0.5*fraglen - 16 #-30
        maxsig = 0.5*fraglen + 94 #+80
...
def main(covfile, order, outfile, outplot, fraglen, width):
    ...
    #theoretically there is at least enough space for region length/ (2*fragment length) peaks. Thus set order to this number
...
if __name__ == '__main__':
    ...
    for peak in open(filesfile):
        ...
        outplot = os.path.join(plotdir, name + '.pdf') #'.png') #pdf is smaller than png
