...
def annotate_human_mouse(peaks, annotfile, outfile):
    ...
    o = open(outfile, 'w')
    o.write('#chrom\tstart\tstop\tpeakID\tZ-score\tstrand\tnearest_upstream_promoter_members\toffset\t2nearest_upstream_promoter_members\toffset\t3nearest_upstream_promoter_members\toffset\tnearest_downstream_promoter_members\toffset\t2nearest_downstream_promoter_members\toffset\t3nearest_downstream_promoter_members\toffset\tgenes[member|symbol|gid|name](##-separated)\n')
    ...
    for line in open(peaks):
        ...
        sorted_list.append(['None', ['None', 'None']]) # to be save if there are less than 3 promoters down- or upstream
        sorted_list.append(['None', ['None', 'None']])
        sorted_list.append(['None', ['None', 'None']])
        ...
        if len(dist_promoter_list_up) + len(dist_promoter_list_down) == 0:
            nonannotated += 1
...
def annotate_drosophila(peaks, annotfile, outfile):
    ...
    for line in open(annotfile):
        if line.startswith('#'):
            continue
        t = line.strip().split('\t')
        annot_dict['%s,%s,%s' %(t[0], t[1], t[2])] = [t[3], t[5]]
    ...
    o = open(outfile, 'w')
    o.write('#chrom\tstart\tstop\tpeakID\tZ-score\tstrand\tnearest_upstream_gene_name|ensembl_id\toffset\t2nearest_upstream_gene_name|ensembl_id\toffset\t3nearest_upstream_gene_name|ensembl_id\toffset\tnearest_downstream_gene_name|ensembl_id\toffset\t2nearest_downstream_gene_name|ensembl_id\toffset\t3nearest_downstream_gene_name|ensembl_id\tdistance\n')
    ...
    for line in open(peaks):
        ...
        for g in annot_dict:
            ...
            if diff > 0: #gene is downstream
                genes_downstream.append([diff, annot_dict[g]])
            else:
                genes_upstream.append([diff, annot_dict[g]])


        sorted_list = sorted(genes_upstream, key = lambda k : k[0], reverse=True)
        sorted_list.append(['None', ['None', 'None']]) # To not get irregularities in the output file when there are less than 3 genes.
        sorted_list.append(['None', ['None', 'None']])
        sorted_list.append(['None', ['None', 'None']])
        nearest_upstream = sorted_list[:3]
