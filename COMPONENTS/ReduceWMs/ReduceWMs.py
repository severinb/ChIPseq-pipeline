...
def execute(cf):
    ...
    score_co = cf.get_parameter("score_cutoff", "float")
    ...
    for i, line in enumerate(open(infile)):
        ...
        if float(t[2]) > score_co:
            wmdict[t[1]] = float(t[2])
        else:
            badwms.append(t[1])
    ...
    #only take 10 best WMs at maximum, otherwise it can take too long.
    if len(final_wms) > 10:
        final_wms = final_wms[:10]
        print 'final WMs list reduced to 10 WMs: ', final_wms


    if keepWM:
        if not keepWM in final_wms:
            final_wms.append(keepWM)
