...
# this script is prepared for Nick Kelley to run bowtie with 3 mismatches.
...
my $bowtieOptions = "-f -p $mthreads -v 3 -a -B 1 --quiet --best --strata"
  ;    # allow up to 2 mismatches, ignore quality scores, find all hits up to m. coord start 1
my $seqBins = [14, 20, 24, 30];    # seqence lengths for 0,1,2,3 mismatches
...
if ($typeOfTarget eq "g")
{
    ...
    foreach my $fileName (@unsortedRalDir)
    {
        if ($fileName =~ /\.ral$/)
        {
			my $ret = fastSortRAL("$tempDir/$fileName", "$outDir/$fileName");
            if($ret)
            {
				die "Sorting of file $tempDir/$fileName failed\n";
			}
